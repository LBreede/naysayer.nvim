...
--  resolve `std`/`core` and infer types. Where that comes from differs per machine,
--  so this file detects rather than assumes:
--
--   - A rustup toolchain ships rust-src as a component and rust-analyzer finds it
--     under the sysroot unaided. Nothing to configure: the branch below is skipped
--     and the server's own defaults are already right.
--   - The rez rust packages ship rustc, cargo, rustfmt and rust-analyzer but *not*
--     rust-src, so it logs
--       "can't load standard library, try installing `rust-src`"
--     and every type resolves to unknown -- which silently kills inlay hints and
--     most code actions while leaving the server attached and apparently fine.
--     There the source is unpacked by hand and pointed at explicitly.
--
--  Keyed on the directory rather than on a hostname, so it is one committed file
--  that is correct on both. It also means a dangling `current` symlink falls back
--  to auto-detection rather than handing the server a path that does not resolve.
--
